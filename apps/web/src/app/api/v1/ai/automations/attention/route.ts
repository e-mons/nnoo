...
export async function GET(request: NextRequest) {
  try {
    ...
    const events = await AutomationService.getAttentionEvents(supabase, businessId, status || undefined);
    ...
    return NextResponse.json(
      { success: false, error: { code: 'AI_INTERNAL_ERROR', message: err?.message || 'Failed to fetch attention events' } },
      { status: 500 }
    );
  }
}
