...
import { RejectBookkeepingInputSchema } from '@nnoo/validation';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const parsed = RejectBookkeepingInputSchema.safeParse({ ...body, classificationId: id });

    if (!parsed.success) {
      return NextResponse.json(
        { error: { code: 'AI_BOOKKEEPER_INVALID_INPUT', message: 'Invalid rejection payload.', details: parsed.error.issues } },
        { status: 400 }
      );
    }
    ...
    const result = await service.rejectSuggestion(parsed.data, {
      businessId,
      userId: user.id,
