...
import { ApplyBookkeepingInputSchema } from '@nnoo/validation';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const parsed = ApplyBookkeepingInputSchema.safeParse({ ...body, classificationId: id });

    if (!parsed.success) {
      return NextResponse.json(
        { error: { code: 'AI_BOOKKEEPER_INVALID_INPUT', message: 'Invalid application payload.', details: parsed.error.issues } },
        { status: 400 }
      );
    }
    ...
    const result = await service.applySuggestion(parsed.data, {
      businessId,
      userId: user.id,
    ...
    if (error instanceof AISafeError) {
      const status =
        error.code === 'AI_BOOKKEEPER_REVIEW_NOT_FOUND' || error.code === 'AI_BOOKKEEPER_TARGET_NOT_FOUND'
          ? 404
          : error.code === 'AI_FORBIDDEN' || error.code === 'AI_BOOKKEEPER_REVIEW_FORBIDDEN'
          ? 403
          : error.code === 'AI_BOOKKEEPER_APPLICATION_CONFLICT'
          ? 409
          : 400;
