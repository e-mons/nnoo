...
export async function GET(request: NextRequest) {
  try {
    ...
    const summary = await NotificationService.getNeedsAttention(supabase, {
      businessId,
      userId: user.id,
    });
    ...
    return NextResponse.json(
      { success: false, error: { code: 'NOTIFICATION_PROCESSING_FAILED', message: err?.message || 'Failed to fetch attention summary' } },
      { status: 500 }
    );
  }
}
