...
export async function GET(request: NextRequest) {
  try {
      ...
      return NextResponse.json(
        {
          ...
          error: {
            ...
            message: 'You must be authenticated to view Business Health Score history.',
            retryable: false,
          },
        },
        { status: 401 }
    ...
    // 3. Retrieve historical snapshots
    const history = await BusinessHealthService.getScoreHistory(
      {
        supabase,
        businessId,
        userId: user.id,
    ...
    return NextResponse.json(
      {
        ...
        error: {
          ...
          message: error.message || 'An unexpected error occurred while retrieving score history.',
