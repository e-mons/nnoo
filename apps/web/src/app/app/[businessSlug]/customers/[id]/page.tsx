...
import { Users, Mail, Phone, MapPin, Building, ArrowLeft, Pencil } from 'lucide-react';
import Link from 'next/link';
import { CustomerActions } from '@/components/customers/customer-actions';

export const metadata: Metadata = {
  title: 'Customer Details | NNOO',
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        <div>
          <Link
            href={`/app/${resolvedParams.businessSlug}/customers`}
          ...
          <h1 className="text-2xl font-bold text-white flex items-center gap-3">
            <Users className="w-7 h-7 text-[#B8F25C]" />
            {customer.name}
        ...
        <div className="flex flex-col sm:flex-row items-end sm:items-center gap-3">
          ...
          />
          <Link
            href={`/app/${resolvedParams.businessSlug}/customers/${customer.id}/edit`}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="text-white font-medium space-y-1">
                {customer.addressLine1 ? (
                  <>
                    ...
                    <p>
                      {[customer.city, customer.state, customer.countryCode].filter(Boolean).join(', ')}
                    </p>
                  </>
                ) : (
        ...
        <div className="bg-[#143628]/40 border border-white/10 rounded-3xl p-6 backdrop-blur-xl md:col-span-2">
          ...
          {customer.notes ? (
            ...
            <p className="text-white/40 italic">No internal notes for this customer.</p>
          )}
          
          <div className="mt-8 pt-6 border-t border-white/10 flex flex-wrap gap-x-8 gap-y-4 text-xs text-white/40">
            <p>Created: {new Date(customer.createdAt).toLocaleDateString('en-US')}</p>
            <p>Last updated: {new Date(customer.updatedAt).toLocaleDateString('en-US')}</p>
            <p>ID: {customer.id}</p>
          </div>
        </div>
      </div>
