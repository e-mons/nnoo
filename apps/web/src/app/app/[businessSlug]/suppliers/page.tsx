...
import { Plus, Users, Search, Settings, Truck } from 'lucide-react';
import { SupplierStatus, SupplierType } from '@nnoo/contracts';

export const metadata: Metadata = {
  title: 'Suppliers | NNOO',
  description: 'Manage your business suppliers.',
};
...
export default async function SuppliersPage({
  ...
  const type: SupplierType | undefined =
    rawType === 'individual' || rawType === 'business' ? rawType : undefined;

  const status: SupplierStatus | 'all' =
  ...
  const { data: suppliers, error, totalCount } = await getSupplierList(business.id, {
    searchQuery: q,
    type,
    status,
    page,
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-white/60 text-sm mt-1">
            Manage your suppliers for {business.name}
          </p>
        </div>
        
        <div className="flex items-center gap-3">
          <Link
            href={`/app/${resolvedParams.businessSlug}/suppliers/new`}
      ...
      <div className="bg-[#143628]/40 border border-white/10 rounded-3xl overflow-hidden backdrop-blur-xl">
        {error ? (
          ...
          <div className="p-16 text-center">
            ...
            <h3 className="text-lg font-medium text-white mb-2">No suppliers found</h3>
            <p className="text-white/50 text-sm max-w-md mx-auto mb-6">
              Get started by adding your first supplier record.
            </p>
            <Link
              href={`/app/${resolvedParams.businessSlug}/suppliers/new`}
              ...
              <tbody className="divide-y divide-white/5">
                {suppliers.map((supplier) => (
                  <tr key={supplier.id} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="text-white/80">{supplier.email || <span className="text-white/30 italic">No email</span>}</div>
                      <div className="text-xs text-white/50 mt-1">{supplier.phone || 'No phone'}</div>
                    </td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center px-2 py-1 rounded-md text-xs font-medium capitalize ${
                        supplier.supplierType === 'business' ? 'bg-amber-500/10 text-amber-400' : 'bg-blue-500/10 text-blue-400'
                      }`}>
                    ...
                    <td className="px-6 py-4 text-right">
                      <Link
                        href={`/app/${resolvedParams.businessSlug}/suppliers/${supplier.id}`}
