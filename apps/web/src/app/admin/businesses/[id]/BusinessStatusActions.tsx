...
import { suspendBusinessAction, restoreBusinessAction } from '../../../../lib/actions/admin';
import { useRouter } from 'next/navigation';

export function BusinessStatusActions({ businessId, currentStatus }: { businessId: string, currentStatus: string }) {
  ...
  const handleAction = async () => {
    ...
    if (currentStatus === 'active') {
      res = await suspendBusinessAction(businessId, reason);
    } else {
      res = await restoreBusinessAction(businessId, reason || 'Administrative restoration');
    }
  ...
  return (
    <div className="mt-6 border-t border-[rgba(255,255,255,0.1)] pt-6">
      ...
      {!showConfirm ? (
        ...
        >
          {currentStatus === 'active' ? 'Suspend Business' : 'Restore Business'}
        </button>
      ) : (
        <div className="bg-[rgba(255,255,255,0.02)] border border-[rgba(255,255,255,0.1)] p-4 rounded-xl max-w-md space-y-4">
          <p className="text-sm text-[rgba(255,255,255,0.8)]">
            Are you sure you want to {currentStatus === 'active' ? 'suspend' : 'restore'} this business?
          </p>
